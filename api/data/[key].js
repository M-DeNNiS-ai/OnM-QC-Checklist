...
export default async function handler(req, res) {
   ...
   * ============================================================
   * These are used by the multi-device frontend so one device
   * never replaces an entire store belonging to another device.
   */
  if (["POST", "PATCH"].includes(req.method)) {
    ...
    if (!record || typeof record !== "object" || Array.isArray(record)) {
      return res.status(400).json({
        error: "Request must contain a record object"
      });
    }
    ...
    try {
      ...
      const { id: ignoredId, ...payload } = record;
      ...
      if (req.method === "PATCH") {
        ...
        if (existingError) {
          console.error("PATCH lookup error:", existingError);
      ...
      await audit(
        supabase,
        user,
        req.method === "POST" ? "CREATE_RECORD" : "UPDATE_RECORD",
        store,
        id,
        null
      );
      ...
      console.error("Record save exception:", error);
      return res.status(500).json({
  ...
  if (req.method === "DELETE") {
    ...
    try {
      ...
      console.error("Record delete exception:", error);
      return res.status(500).json({
        error: error.message || "Internal server error"
      });
    }
